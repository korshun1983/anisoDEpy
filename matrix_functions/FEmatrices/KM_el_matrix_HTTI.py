...
def KM_el_matrix_HTTI(BasicMatrices, CompStruct:BigCompStruct, FEMatrices, ii_d, ii_el, ElPhysProps, TriProps):
    ...
    #  K1_m ab :::  = conv(np.conj(B1_m ia :::), conv(C_ij :::, B1_m jb :::) ) (3Nn x 3Nn matrix)
    #  K2_m = B1_adjoint * C_m * B2 - B2_adjoint * C_m * B1
    #  K2_m ab :::  = conv(np.conj(B1_m ia :::), conv(C_ij :::, B2_m jb :::) )
    #                   - conv(np.conj(B2_m ia :::), conv(C_ij :::, B1_m jb :::) ) (3Nn x 3Nn matrix)
    #  K3_m = B2_adjoint * C_m * B2
    #  K3_m ab :::  = conv(np.conj(B2_m ia :::), conv(C_ij :::, B2_m jb :::) ) (3Nn x 3Nn matrix)
    #  M_m = N_adjoint * Rho_m * N
    #  M_m ab :::  = conv(np.conj(N_m ia :::), conv(C_ij :::, N_m jb :::) ) (3Nn x 3Nn matrix)
    #
    ...
    for kk in range (0,N_nodes):
        ...
        for ii in range(0,3):
            LxTCijLz_dxL_large[:, kk,:] = np.kron(OnesMatrix, (LxTCijLz_kk * dxL[ii] + LyTCijLz_kk * dyL[ii] ))
            ...
            for jj in range(0,3):
                ...
                LxTCijLy_kk * dxL[ii] * dyL(jj) + LyTCijLx_kk * dyL[ii] * dxL(jj) + LyTCijLy_kk * dyL[ii] * dyL(jj) ))
    ...
    ElMatrices.B1tCB1Matrix = np.squeeze(np.sum(np.sum(np.sum(B1tCB1Matrix_unsummed, axis = 0), axis = 2), axis = 3))
